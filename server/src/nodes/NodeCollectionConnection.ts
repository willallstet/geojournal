...
export class NodeCollectionConnection {
  ...
  async findNodesBySearch(searchTerm: string) {
    const foundNodes: INode[] = []
    await this.client
      .db()
      .collection(this.collectionName)
    ...
    await this.client
      ...
      .find({
        ...
        $text: { $search: searchTerm.replace(/<\/?[^>]+(>|$)/g, '') },
      })
      .project({ score: { $meta: 'textScore' } }) // needed according to this (? https://stackoverflow.com/questions/48975707/mongoerror-must-have-meta-projection-for-all-meta-sort-keys-using-mongo-db-na), was throwing me errors in test otherwise
